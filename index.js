...
app.get("/v2/dashboard/:year", (req, res) => {
  ...
  const results = {
    months,
    "year": tools.generateYearData(filteredYearData, months),
  };

  months.map((month) => {
    results[month] = tools.generateMonthData(monthData(month));
  });
...
app.get("/v3/data/:year/:month", (req, res) => {
  ...
  const monthData = tools.getMonthData(year, month);
...
app.get("/v3/dashboard/:year", (req, res) => {
  ...
  const monthData = (month) => {
    return tools.getMonthData(year, month);
  };
  ...
  const results = {
    months,
    "year": tools.generateYearData(yearData, months),
  };
